...
- Take a closer look on function for variant ~7> Proceed~ of the second menu.
  #+begin_src c 
int __fastcall proceed(unsigned int fd, Cart *items, const char *session)
{
...
- The function corresponding to ~5> Change amount~:
  #+begin_src c -n -r
void __fastcall change_amount(int fd, Cart *items, const char *session)
{
  ...
  ~find_item_or_empty~:
  #+begin_src c
__int64 __fastcall find_item_or_empty(Cart *items, const char *item_name, const char *path)
{
...
So now we just want the call on [[(critical)]] line to return ~-2~, and the
~items[-2]~ is exactle ~NEED_TO_BE_NON_ZERO~ field, which we then sets to
non zero value.
